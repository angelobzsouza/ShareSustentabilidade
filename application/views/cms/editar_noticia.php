...
<html>
  ...
  <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.4.0/css/font-awesome.min.css" rel="stylesheet" type="text/css" />
	...
	<header id="fh5co-header" class="fh5co-cover fh5co-cover-sm fundo-editar-noticia" role="banner">
	...
	<section id="fh5co-blog" class="mt-0">
		<div class="container">
			<div class="row justify-content-center">
				<div class="col-12 text-center">
					<form id="form_cadastro" method="POST" action="<?= base_url('cms/noticias/update') ?>" enctype="multipart/form-data">
						<!-- Enviar o id por post -->
						<input type="hidden" name="noticia_id" id="noticia_id" value="<?= $noticia->ID ?>">
						<div class="form-row mb-3">
							<div class="col-md-5 text-md-left text-center">
								<div class="container-img-noticia">
									<img src="<?= base_url('assets/uploads/images/'.$noticia->Foto) ?>" class="mb-4 mb-md-0 image-noticia" alt="imagem-noticia" onclick="selecionaAlterarFoto()" name="iamgem-noticia" id="imagem-noticia"/>
									<div class="middle">
								    <div class="text-img" onclick="selecionaAlterarFoto()">Alterar</div>
								  </div>
								  <input type="file" class="sr-only" name="foto_input" id="foto_input">
							  </div>
							</div><!-- /col-md-6 -->
							<div class="col-md-7 text-left">
								<input type="text" class="form-control mb-3" placeholder="Título" id="titulo" name="titulo" required maxlength="50" value="<?= $noticia->Titulo ?>">
								<textarea id="previa" name="previa" class="form-control" rows="8" placeholder="Escreva uma prévia da notícia..." maxlength="500" required><?= $noticia->Previa ?></textarea>
							</div><!-- /col-md-6 -->
						</div><!-- /form-row -->
								<textarea id="texto" name="texto" class="form-control" rows="10" required><?= $noticia->Texto ?></textarea>
