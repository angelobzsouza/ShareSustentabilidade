...
<html>
	...
	<title>Resultados <?= $teste->Nome ?></title>
	...
	<header id="fh5co-header" class="fh5co-cover fh5co-cover-sm fundo-resultados" role="banner">
			...
			<?php if(empty($resultados)) :?>
				<div class="row">
					<div class="col-md-12 text-center">
						<h1>Ainda não há respostas para esse teste</h1>
					</div>
				</div>
			<?php else : ?>
				<table class="table text-center">
					...
					<tbody>
						<?php foreach($resultados as $resultado)  {?>
							<tr id="linha<?= $resultado->ID ?>">
								<td><?= $resultado->Nome ?></td>
								<td><?= $resultado->Email ?></td>
								<td><?= number_format($resultado->Resultado*100, 2, ",", ".") ?>%</td>
								<td><button type="button" class="link text-danger" onclick="excluiResultado(<?= $resultado->ID ?>)">Excluir</button></td>
							</tr>
						<?php } ?>
					</tbody>
				</table>
