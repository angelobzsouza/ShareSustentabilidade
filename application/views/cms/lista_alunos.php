...
<html>
	...
	<header id="fh5co-header" class="fh5co-cover fh5co-cover-sm fundo-lista-alunos" role="banner">
	...
	<section id="fh5co-blog">
		<div class="container">
			<h5 class="text-right"><a href="<?= base_url('cms/pessoas/cadastraAluno') ?>" class="link text-primary">Novo Aluno</a></h5>
			<table class="table">
				<thead>
					<tr>
						<th>Nome</th>
				...
				<tbody>
					<?php foreach($alunos as $aluno)  {?>
						<tr id="linha<?= $aluno->ID ?>">
							<td><?= $aluno->Nome ?></td>
							<td><?= $aluno->Email ?></td>
							<td><button class="link text-primary" onclick="tornarProfessor(<?= $aluno->ID ?>)">Tornar Professor</button></td>
							<td><button class="link text-danger" onclick="excluiAluno(<?= $aluno->ID ?>)">Excluir</button></td>
						</tr>
					<?php } ?>
				</tbody>
			</table>
